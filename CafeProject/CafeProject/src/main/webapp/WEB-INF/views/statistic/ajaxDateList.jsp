	        ...
	        <thead>
	            <tr role="row">
		            ...
		            <th class="sorting" tabindex="0" aria-controls="editabledatatable" rowspan="1" colspan="1" 
	                	aria-label="">
	                    <a id="orderDate" href="#" class="table-sort">
							<spring:message code="cafeOrder.orderDate" />
							<c:if test="${!empty orderField && orderField == 'orderDate'}">
	                ...
	                <th class="sorting" tabindex="0" aria-controls="editabledatatable" rowspan="1" colspan="1" 
	                	aria-label="">
	                    <spring:message code="cafeOrder.firstOrder" />
	                </th>
	                
	                <th class="sorting" tabindex="0" aria-controls="editabledatatable" rowspan="1" colspan="1" 
	                	aria-label="">
	                    <spring:message code="cafeOrder.lastOrder" />
	                </th>
	            </tr>
	        </thead>
	                ...
	                <c:forEach items="${cafeOrderList}" var="item">
	                    <tr role="row" class="odd cf_order_record">
	                        ...
	                        <td>
	          				...
	          				<td>
	          					<div>${item.firstOrderTime}</div>
	          				</td>
	          				<td>
	          					<div>${item.lastOrderTime}</div>
	          				</td>
	                    </tr>
	                </c:forEach>
	            </c:if>
...
<script type="text/javascript">
	j(document).ready(function() {
		...
		j("#orderDate").click(function(){
			searchCafeOrderModeDateSort('orderDate',j('#sort').val());
		});
		
		j("#numOfFood").click(function(){
			searchCafeOrderModeDateSort('numOfFood',j('#sort').val());
		});
		
		j("#totalMoney").click(function(){
			searchCafeOrderModeDateSort('totalMoney',j('#sort').val());
		});
		
		j("#totalProfit").click(function(){
			searchCafeOrderModeDateSort('totalProfit',j('#sort').val());
		});
		
		function searchCafeOrderModeDateSort(orderField,sort){
    		dialogWating.open();
			j.ajax({
	   	        type: "POST",
